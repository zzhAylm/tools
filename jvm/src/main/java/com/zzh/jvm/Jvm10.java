...
public class Jvm10 {
    ...
    public static void main(String[] args) {
        ...
        //动态链接：（帧数据区包含方法返回地址，动态链接，一些附加信息）
            ...
            // 早期绑定和晚期绑定（动态链接和静态链接）：多态就是晚期绑定，一个父类的引用指向子类的实例
            // 非虚方法：静态方法，私有方法，final方法，父类的方法，实例构造器   其他的都是虚方法
            // invokedynamic指令，编译器无法确定类型，运行期确定类型，如lamda表达式


        //方法返回地址：
            // return address
            // 被调用的方法执行完成之后，会将方法返回地址交给执行引擎，执行引擎以次地址来确定 当前方法需要从哪里开始执行
        ...
        // 虚拟机栈的面试题：
            // 举例栈溢出的情况？ StackOverflowError ,-Xss1024k,设置栈的大小，当无法给栈分配内存会出现OOM
            // 方法中的局部变量是否存在线程安全问题？
                // 如果变量是在方法内声明创建的则是线程安全的，
                // 如果变量（引用类型）是以参数的形式传入方法中，则可能存在线程安全问题
                // 如果变量（引用类型 ）是方法返回的，则也能存在线程安全问题
